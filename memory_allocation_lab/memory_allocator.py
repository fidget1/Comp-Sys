...
class MemoryAllocator:
    ...
    """
    def myalloc(self, block_size, ptr_num):
        free_block = self.heap.find_free_block(block_size)
        nodes = self.heap.split(free_block, block_size, self.heap_size, ptr_num)
        if nodes is None:
            print("error - neither False nor [node1, node2]")
        elif not nodes:
            print("warning - extending memory")
            self.mysbrk(free_block.address, block_size)
    ...
    def myfree(self, ptr_num):
        ...
        block.allocated = 0
        block.ptr_num = None
        block = self.heap.join_adjacent_blocks(block)
